...
macOS 上で **Kindle アプリ**を前面に置いた状態から、画面を連続でスクリーンショットし、PNG として保存するスクリプトです。ページ送りの向き（左送り＝縦書き・右綴じ / 右送り＝横書き・左綴じ）は `--key auto` で自動判定できます。
...
| `--key` | ページ送りキー（`auto` / `left` / `right`）。`auto` は向きを自動判定 |
...
### ページ送りの向きを自動判定する（`--key auto`）

本によって次ページが **左送り**（縦書き・右綴じ）か **右送り**（横書き・左綴じ）かが違います。`--key auto` を付けると、次の 2 段階で自動判定します。

1. **端の試し打ち** — 左右を 1 回ずつ押す。片方しか効かなければ本の端にいる。ただし「先頭」と「巻末」はこれだけでは区別できない（先頭では戻る側が、巻末では進む側が効かず、症状が鏡像になる）ので、**効いたキーを送り方向と決めつけない**。
2. **本文レイアウト** — ページを送りながら、行間に空行ができる（＝横書き）か、列間に空列ができる（＝縦書き）かを最大 40 ページ分見て投票する。縦書きの本でも表組みページは横書きに見えるため、**反対票の 2 倍を超えて 4 票**集まって初めて確定とする。

縦書きと判定されれば右綴じ＝`left` 送り、横書きなら左綴じ＝`right` 送りになります。**どちらとも判断できない場合は撮影を始めずに停止する**ので、逆向きに大量撮影する事故は起きません（判定できなかったときは `--key left` / `--key right` で明示してください）。

判定ロジックを変更したら、撮影済みフォルダを仮想の本として使う回帰テストを実行してください。実機もキー操作も不要です。
...
`--rewind` を付けると、撮影前に戻るキーを押し続けて本の先頭まで戻します。画面が変化しなくなった時点を先頭と判断するので、総ページ数もメニュー構成も知らなくて構いません。`--key auto --rewind --stop-repeat 3` を組み合わせると、**本のどこを開いていても全ページを撮り切れます**。

```bash
# 開いている本を、向きも開始位置も自動で判断して全ページ撮影
python capture_kindle.py --pages 999 --key auto --rewind \
...
```text
/capture-book              # 開いている本を全ページ撮影
/capture-book 120          # 120 ページ撮影
/capture-book 120 2        # 120 ページを 2 秒間隔で撮影
```

実行すると `--key auto --rewind --app --owner --stop-repeat 3` を付けてスクリプトが呼ばれ、開始前に「Kindle を最前面にする」案内が表示されます。撮影後は 001.png が表紙になっているかの確認まで行います。
